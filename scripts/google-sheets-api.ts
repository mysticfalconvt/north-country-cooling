...
export async function getSheetsDataFromGoogle() {
  try {
    ...
    if (rows?.length) {
      ...
      console.log('Google Sheets data retrieved:', dataToReturn);
      return dataToReturn;
    }
  } catch (err) {
...
export async function getLinksDataFromGoogle() {
  try {
    const scopes = ["https://www.googleapis.com/auth/spreadsheets"];
    const jwt = new google.auth.JWT(
      process.env.GOOGLE_SHEETS_CLIENT_EMAIL,
    ...
    if (rows?.length) {
      ...
      console.log('Google Sheets links retrieved:', dataToReturn);
      return dataToReturn;
    }
  } catch (err) {
    console.log('Google Sheets links error:', err);
  }
